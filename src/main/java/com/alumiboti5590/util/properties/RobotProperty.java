...
public enum RobotProperty {
    ...
    SHOULDER_BRAKE_SOLENOID_A(Integer.class),
    SHOULDER_BRAKE_SOLENOID_B(Integer.class),

    SHOULDER_BRAKE_INVERT(Boolean.class),

    // ------------------
    // Grabber properties
    // ------------------

    GRABBER_SOLENOID_A(Integer.class),
    GRABBER_SOLENOID_B(Integer.class),
    GRABBER_INVERT(Boolean.class);
    ...
    private void validatePropertyType(Class<? extends Object> classType) {
        if (this.propertyType != classType) {
            ...
            String err = String.format(
                    "Tried to call `%s.%s()` when it is a `%s`", this.name(), methodName, classType.getName());
