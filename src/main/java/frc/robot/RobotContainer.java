...
public class RobotContainer {
    ...
    private void configureSubsystems() {
        ...
        this.shoulder = new Shoulder(arm::armExtendedToUnsafeMoveDistance, arm::getShoulderFeedForwardMultiplier);
    }

    /** Configure trigger & axis bindings between the robot and the controllers */
    private void configureBindings() {
        driverController = new XboxDriverController(
                Constants.Controller.DRIVER_CONTROLLER_PORT, Constants.Controller.XBOX_CONTROLLER_DEADBAND);
        operatorController = new XboxOperatorController(
                Constants.Controller.OPERATOR_CONTROLLER_PORT, Constants.Controller.XBOX_CONTROLLER_DEADBAND);
        ...
        driverController
                .getStraightDrive()
                .whileTrue(new StraightDrive(drivetrain, () -> driverController.getTankDriveValues().first));
        ...
        // Grabber Controls
        // ~~~~~~~~~~~~~~~~
        operatorController.getGrabOpen().whileTrue(run(() -> this.grabber.setGrabMode(GrabMode.OPEN), this.grabber));
        operatorController.getGrabClose().whileTrue(run(() -> this.grabber.setGrabMode(GrabMode.CLOSE), this.grabber));
        ...
        operatorController
                .getLowGoal()
                .whileTrue(run(() -> this.shoulder.setGoalPosition(ShoulderPosition.LOW_CUBE), this.shoulder));
        operatorController
                .getMedGoal()
                .whileTrue(run(() -> this.shoulder.setGoalPosition(ShoulderPosition.MED_CUBE), this.shoulder));
        operatorController
                .getHighGoal()
                .whileTrue(run(() -> this.shoulder.setGoalPosition(ShoulderPosition.HIGH_CUBE), this.shoulder));
        ...
        operatorController
                .getLowGoal()
                .and(operatorController::getGoalModifier)
                .whileTrue(run(() -> this.shoulder.setGoalPosition(ShoulderPosition.LOW_CONE), this.shoulder));
        operatorController
                .getMedGoal()
                .and(operatorController::getGoalModifier)
                .whileTrue(run(() -> this.shoulder.setGoalPosition(ShoulderPosition.LOW_CONE), this.shoulder));
        ...
        operatorController.getShoulderZeroMode().whileTrue(run(() -> this.shoulder.startZeroingMode(), this.shoulder));
    ...
    private void configureDefaultCommands() {
        ...
        setDefaultCommand(
                ...
                () -> shoulder.controllerAction(
                        operatorController.getShoulderModifier(), operatorController.getShoulderSafetyOverride()));
    }
    ...
    private void configureAutoCommands() {
        ...
        autoChooser.addOption("Score Low & Leave Community", new ScoreLowAndLeaveCommunity(drivetrain, intake));
